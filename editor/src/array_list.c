...
#define ERROR_EXIT(x, txt) fprintf(stderr, txt); exit(x);
...
void array_list_append(array_list *self, void *data) {
    if (self->len == self->capacity) {
        ...
        if (!new_data) { ERROR_EXIT(-1, "Cannot allocate memory for array list\n"); }
        self->data = new_data;
    }

    memcpy(self->data + self->len * self->data_size, data, self->data_size);
    self->len++;
}
...
void array_list_remove(array_list *self, size_t index) {
	...
	memcpy(self->data + index * self->data_size, self->data + self->len * self->data_size, self->data_size);
}

void array_list_pop_first(array_list *self) {
    memcpy(self->data, self->data + 1 * self->data_size, (self->len -1) * self->data_size);
    self->len--;
}
