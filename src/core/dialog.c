...
static DialogText* _get_new_dialog_text(char *txt) {
...
static DialogQuestion* _get_new_dialog_question(char *question, char *answer[4], char *wrong_txt, char *correct_txt, u8 correct_idx) {
...
void dialog_append(Dialog *dialog, char *name, enum DialogType type, void *data) {
    ...
    LOG_DEBUG("Dialog: Append dialog -> %s - %p - %d", (node->type == DIALOG_TYPE_TEXT) ? "Text" : "Question", node->next, dialog->length);
}
        ...
        if (curr->type == DIALOG_TYPE_QUESTION) {
            if(window_get_key(global.window, GLFW_KEY_SPACE)) {
                if(curr->type == DIALOG_TYPE_QUESTION){
                    ...
                    DialogNode *node = malloc(sizeof(*node));
                    node->name = malloc(strlen(curr->name) + 1);
