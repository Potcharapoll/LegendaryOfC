...
array_list* array_list_init_from_list(array_list *list) {
  array_list *al = malloc(sizeof(*al));
  ASSERT(al != NULL, "Failed to allocate memory for array_list", __FILE__, __LINE__);

  al->len       = 0;
  al->capacity  = list->capacity;
  al->data_size = list->data_size;
  al->len       = list->len;
  al->data      = malloc(list->capacity * list->data_size);
  ASSERT(al->data != NULL, "Failed to allocate memory for array_list entry", __FILE__, __LINE__);

  memcpy(al->data, list->data, list->len * list->data_size);
