...
void player_init(void) {
  ...
  adef_walk[DOWN]  = animation_definition_create(global.animations, player_spritesheet, (f32[]){0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05}, (u8[]){2,2,2,2,2,2,2,2},(u8[]){0,1,2,3,4,5,6,7}, 8);
  adef_walk[UP]    = animation_definition_create(global.animations, player_spritesheet, (f32[]){0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05}, (u8[]){1,1,1,1,1,1,1,1},(u8[]){0,1,2,3,4,5,6,7}, 8);
  adef_walk[RIGHT] = animation_definition_create(global.animations, player_spritesheet, (f32[]){0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05}, (u8[]){0,0,0,0,0,0,0,0},(u8[]){0,1,2,3,4,5,6,7}, 8);
  adef_walk[LEFT]  = animation_definition_create(global.animations, player_spritesheet, (f32[]){0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05}, (u8[]){0,0,0,0,0,0,0,0},(u8[]){0,1,2,3,4,5,6,7}, 8);

  astate_walk[DOWN]  = animation_state_create(global.animations, adef_walk[DOWN], ANIMATION_LOOP);
  astate_walk[UP]    = animation_state_create(global.animations, adef_walk[UP],   ANIMATION_LOOP);
  astate_walk[RIGHT] = animation_state_create(global.animations, adef_walk[RIGHT],ANIMATION_LOOP);
  astate_walk[LEFT]  = animation_state_create(global.animations, adef_walk[LEFT], ANIMATION_LOOP | ANIMATION_FLIPPED);
  astate_idle[DOWN]  = animation_state_create(global.animations, adef_idle[DOWN], 0);
  astate_idle[UP]    = animation_state_create(global.animations, adef_idle[UP], 0);
  astate_idle[RIGHT] = animation_state_create(global.animations, adef_idle[RIGHT], 0);
  astate_idle[LEFT]  = animation_state_create(global.animations, adef_idle[LEFT], ANIMATION_FLIPPED);
...
void player_input(void) {
  ...
  if (update) {
    AnimationState *state = player_get_animation();
    state->flag |= ANIMATION_UPDATE;
    update = false;
  }
...
void player_get_tex_coord(f32 *tex_coord) {
  ...
  if ((astate->flag & ANIMATION_FLIPPED) == ANIMATION_FLIPPED) {
